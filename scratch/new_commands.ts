...
export async function fetchTikTokData(url: string) {
    try {
        ...
        const response = await fetch(\\?url=\&hd=1\);
...
function generateMockQualities(ttData: any, meta: any) {
    const originalRes = meta ? \\x\\ : '1080x1920';
    const originalFps = meta ? meta.fps : 30;
    const codecStr = meta && meta.codec === 'avc1' ? 'h264' : (meta && (meta.codec === 'hev1' || meta.codec === 'hvc1') ? 'hevc' : 'h264');
...
export async function processCheckCommand(url: string) {
    try {
        ...
        const regionName = new Intl.DisplayNames(['en'], { type: 'region' }).of(ttData.region || 'US') || ttData.region;
